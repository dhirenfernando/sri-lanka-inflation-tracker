...
<main>
  ...
  <section><h2>Inflation trends</h2><div class="controls" aria-label="Chart date range"><strong>View:</strong><button data-range="12">1Y</button><button data-range="24" class="active">2Y</button><button data-range="36">3Y</button><button data-range="all">All</button><label>From <input id="from" type="month"></label><label>To <input id="to" type="month"></label></div>
...
const charts={{}}; const color={{CCPI:'#1666a8',NCPI:'#d95732',PPI:'#27835c'}}; let rangeMonths=DEFAULT_RANGE_MONTHS;
...
const maxPeriod=DATA.reduce((latest,row)=>row.period>latest?row.period:latest,DATA[0].period); const minPeriod=DATA.reduce((earliest,row)=>row.period<earliest?row.period:earliest,DATA[0].period);
...
function selectedBounds() {{ const from=document.querySelector('#from').value; const to=document.querySelector('#to').value; if(from&&to) return [from,to]; if(rangeMonths==='all') return [minPeriod.slice(0,7),maxPeriod.slice(0,7)]; return [monthShift(maxPeriod,rangeMonths),maxPeriod.slice(0,7)]; }}
function filtered() {{ const [from,to]=selectedBounds(); return DATA.filter(row=>row.period.slice(0,7)>=from&&row.period.slice(0,7)<=to); }}
...
function renderCharts() {{ const visible=filtered(); line('ccpi-chart',[{{label:'CCPI',color:color.CCPI,rows:rows('CCPI','yoy',visible)}}],'YoY (%)'); line('ncpi-chart',[{{label:'NCPI',color:color.NCPI,rows:rows('NCPI','yoy',visible)}}],'YoY (%)'); line('comparison-chart',[{{label:'CCPI',color:color.CCPI,rows:rows('CCPI','yoy',visible)}},{{label:'NCPI',color:color.NCPI,rows:rows('NCPI','yoy',visible)}}],'YoY (%)'); line('ppi-index-chart',[{{label:'PPI',color:color.PPI,rows:rows('PPI','index',visible)}}],'Index level'); line('ppi-yoy-chart',[{{label:'PPI',color:color.PPI,rows:rows('PPI','yoy',visible)}}],'YoY (%)'); }}
function renderTable() {{ const visible=filtered().slice().sort((a,b)=>b.period.localeCompare(a.period)||a.series.localeCompare(b.series)||a.metric.localeCompare(b.metric)); document.querySelector('#table-summary').textContent=`${{visible.length}} observations in selected range`; document.querySelector('#recent').innerHTML=visible.slice(0,60).map(row=>`<tr><td>${{row.series}}</td><td>${{fmtPeriod(row.period)}}</td><td>${{row.metric.toUpperCase()}}</td><td>${{row.metric==='index'?fmtIndex(row.value,row.series):fmtPct(row.value)}}</td></tr>`).join(''); return visible; }}
function render() {{ const [from,to]=selectedBounds(); document.querySelector('#from').value=from; document.querySelector('#to').value=to; document.querySelectorAll('[data-range]').forEach(button=>button.classList.toggle('active',String(rangeMonths)===button.dataset.range)); renderCharts(); renderTable(); }}
document.querySelectorAll('[data-range]').forEach(button=>button.addEventListener('click',()=>{{rangeMonths=button.dataset.range==='all'?'all':Number(button.dataset.range); document.querySelector('#from').value=''; document.querySelector('#to').value=''; render();}})); document.querySelector('#from').addEventListener('change',()=>{{rangeMonths=null;render();}}); document.querySelector('#to').addEventListener('change',()=>{{rangeMonths=null;render();}});
document.querySelector('#download').addEventListener('click',()=>{{ const rows=renderTable(); const csv=['series,period,metric,value',...rows.map(row=>`${{row.series}},${{row.period}},${{row.metric}},${{row.value}}`)].join('\\n'); const link=document.createElement('a'); link.href=URL.createObjectURL(new Blob([csv],{{type:'text/csv'}})); link.download='sri_lanka_inflation.csv'; link.click(); setTimeout(()=>URL.revokeObjectURL(link.href),0); }});
